...
\item[N] Uh, nicht so prima. Jetzt entwickele ich doch schon seit Jahren Software, und eigentlich hätte ich erwartet, dass es irgendwann mal besser klappt, aber irgendwie lande ich bei jedem Projekt wieder bei den gleichen Problemen. Erst fängt man ganz gut an, aber mit der Zeit wird immer alles mühsamer... Guck mal zum Beispiel unser aktuelles Projekt:
\end{description}
}
...
\includegraphics[width=.45\framewidth]{pics/evans.pdf}
\hfill
\includegraphics[width=.45\framewidth]{pics/vernon.pdf}
\end{frame}
...
\item[Klingt gut, erzähl doch mal!]
\end{description}
}
...
\item[J] Nur das nicht. Dieser Reflex ist leider sehr häufig. Daher habe ich auch bisher die Services nicht erwähnt. Sie sind eher das Mittel für den Notfall. 
\end{description}
}

\note{
\begin{description}
\item[J] Nur wenn Du keine Entität oder kein Value Object findest, wo die Logik hingehört, solltest Du über einen Service nachdenken. 
\item[N] Wieso denn? Ich könnte doch einfach mal die Prozesse runterschreiben in Service-Methoden und dann nach und nach Entities und Value Objects erkennen und herausziehen.
\item[J] Das klappt leider nur sehr selten. Meist bleibt der Code dann sehr prozedural. Die Klassen werden dann zwar meist noch angelegt. Die Logik bleibt aber oft im Service. Das Domänen-Modell verkommt dann zu einem reinen Datenmodell und bleibt blutleer.
\end{description}
}

\note{
\begin{description}
\item[N] Das habe ich verstanden. Ich will meine Daten und mein Verhalten möglichst eng beieinander haben. Das wird mir nicht gelingen, wenn ich vieles in zustandslose Services verpacke. Danke für die Hinweise!
\end{description}
}
...
\item[N] Da haben wir ja schon drüber geredet. Da heißt es dann umbenennen. Das hatten wir auch schon des öfteren.
\end{description}
}

\note{
\begin{description}
\item[N] Bei uns im Team machen wir das so. Wenn bei uns der Verdacht aufkommt, dass wir einen Begriff ersetzen sollten, dann nehmen wir den erstmal auf eine Liste auf und beobachten das ein wenig für uns. Die eigentliche Ersetzung ziehen wir dann mal möglichst schnell durch, wenn es ein wenig ruhiger ist. Dann haben wir nicht ganz so viel zu Mergen. 
\item[J] Gute Idee, das so anzugehen. Das müssen wir auch mal ausprobieren. Oft geht es bei den Änderungen auch über Benennungen hinaus.
\end{description}
}
